...
class DigitSpotter:
    ...
    def run(self, image, threshold=0.7, do_nms=True, show_result=True, nms_threshold=0.3):
        ...
        Returns
        ----------
        bbs : np.ndarray, shape of (N, 4)
            detected bounding box. (y1, y2, x1, x2) ordered.
        
        probs : np.ndarray, shape of (N,)
        ...
        key = None
        if show_result:
            key = self._show_detection_results(image, bbs, y_pred, probs_.max(axis=1))
    ...
    def _show_detection_results(self, image, bboxes, labels, scores):
        # Add extra height for labels visibility
        topPaddingHeight = 20
        if len(image.shape) == 3:
            topPaddingShape = topPaddingHeight, image.shape[1], 3
        else:
            topPaddingShape = topPaddingHeight, image.shape[1]
        topPadding = np.zeros(topPaddingShape, np.uint8)

        image = np.vstack([topPadding, image])
        for bb, label in zip(bboxes, labels):
            y1, y2, x1, x2 = bb
            y1 = y1 + topPaddingHeight
            y2 = y2 + topPaddingHeight
            bb = y1, y2, x1, x2
