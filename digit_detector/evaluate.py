...
class Evaluator(object):
    ...
    def run(self, test_image_files, do_nms=True):
        ...
        for i, image_file in enumerate(test_image_files):
            ...
            detected_bbs, detected_probs_ = self._detector.run(test_image, threshold=0.5, do_nms=do_nms,
                                                               nms_threshold=0.1, show_result=False)
    ...
    def _print_msg(self, recall, precision, f1_score):
        print(f"recall value : {recall}, precision value : {precision}, f1_score : {f1_score}")
